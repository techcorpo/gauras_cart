...
export default function ManufacturerOrders() {
  ...
  useEffect(()=>{ load(); },[]);
  async function setStatus(id,st){ if(st==='cancelled'&&!confirm('Cancel this PO?'))return; try{ await Orders.setStatus(id,st); toast('Marked '+st); load(); }catch(e){toast(e.message);} }
  async function markPaid(id){ try{ await Orders.setPayment(id,'paid'); toast('Marked paid'); load(); }catch(e){toast(e.message);} }
  ...
  return (
    <FodderShell role="manufacturer" title="Distributor Purchase Orders" description="Confirm, ship or deliver — status flows down to farmers automatically.">
      {pos.length===0 && <div className="card p-10 text-center text-slate-400">No distributor orders yet.</div>}
          ...
          <div key={po.id} className="card overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-[#eef2ef] dark:border-[#24332b] flex-wrap gap-2">
              ...
              <div className="flex gap-2">
                <span className={`pill ${pill(po.status)}`}>{po.status}</span>
                <span className={`pill ${po.payment_status==='paid'?'bg-[#e5f5eb] text-brand':'bg-[#fff4dc] text-[#9a6a12]'}`}>{po.payment_status}</span>
              </div>
            </div>
            <table className="w-full text-sm">
              <thead className="bg-[#fbfdfc] dark:bg-[#101a15] text-slate-500 text-xs uppercase"><tr>{['Product','Qty','Unit Price','Subtotal'].map(h=><th key={h} className="text-left p-3">{h}</th>)}</tr></thead>
              <tbody>
            ...
            <div className="flex items-center justify-between p-4 bg-[#fbfdfc] dark:bg-[#101a15] flex-wrap gap-2">
              <span className="font-extrabold text-brand">Total: ₹{Number(po.total_amount).toFixed(2)}</span>
              <div className="flex gap-2 flex-wrap">
                {po.payment_status!=='paid' && <button className="btn btn-secondary h-9 px-3 text-xs" onClick={()=>markPaid(po.id)}>Mark Paid</button>}
                {(NEXT[po.status]||[]).map(st=>(
                  <button key={st} className={`h-9 px-3 text-xs rounded-lg font-bold ${st==='cancelled'?'btn-secondary text-rose-600':st==='confirmed'?'btn btn-primary':'btn btn-secondary'}`} onClick={()=>setStatus(po.id,st)}>{st[0].toUpperCase()+st.slice(1)}</button>
                ))}
              </div>
            </div>
          </div>
