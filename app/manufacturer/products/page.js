...
export default function ManufacturerProducts() {
  ...
  async function save(){
    const b = edit;
    if(!b.sku || !b.name || b.base_price===''||b.base_price==null){ toast(t('SKU, name, base price required')); return; }
    try { if(b.id) await Products.update(b.id,b); else await Products.create(b); toast(t('Saved')); setEdit(null); load(); }
    catch(e){ toast(e.message); }
  }
  async function del(id){ if(!confirm(t('Delete product?')))return; try{ await Products.remove(id); toast(t('Deleted')); load(); }catch(e){toast(e.message);} }
  ...
  async function savePricing(){
    ...
    try { await Products.savePricing(pricing.product.id, prices); toast(t('Pricing saved')); setPricing(null);}catch(e){toast(e.message);}
  }

  return (
    <FodderShell role="manufacturer" title="Products" description="Your products and per-distributor pricing"
      action={<button className="btn btn-primary" onClick={()=>{setEdit({ unit:'bag', is_active:true });setCatResults([]);setCatQ('');}}>{t('+ Add Product')}</button>}>
      <div className="card overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-[#fbfdfc] dark:bg-[#101a15] text-slate-500 text-xs uppercase">
            <tr>{['SKU','Name','Category','Unit','Base Price','Status','Actions'].map(h=><th key={h} className="text-left p-3">{t(h)}</th>)}</tr>
          </thead>
          <tbody>
            {rows.length===0 && <tr><td colSpan={7} className="p-8 text-center text-slate-400">{t('No products yet.')}</td></tr>}
            {rows.map(p=>(
              <tr key={p.id} className="border-t border-[#f0f3f1] dark:border-[#24332b]">
                ...
                <td className="p-3"><span className={`pill ${p.is_active?'bg-[#e5f5eb] text-brand':'bg-[#fdecec] text-[#b64b4b]'}`}>{p.is_active?t('Active'):t('Inactive')}</span></td>
                <td className="p-3 space-x-1">
                  <button className="btn btn-secondary h-8 px-2 text-xs" onClick={()=>{setEdit({...p});setCatResults([]);setCatQ('');}}>{t('Edit')}</button>
                  <button className="btn btn-secondary h-8 px-2 text-xs" onClick={()=>openPricing(p)}>{t('Pricing')}</button>
                  <button className="btn btn-secondary h-8 px-2 text-xs text-rose-600 border-rose-200" onClick={()=>del(p.id)}>{t('Delete')}</button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {edit && (
        <Modal title={edit.id?t('Edit Product'):t('Add Product')} onClose={()=>setEdit(null)} onSave={save}>
          <div className="space-y-3">
            <div>
              <div className="label">{t('Catalog Product')} <span className="font-normal text-slate-400">{t('(standardizes search)')}</span></div>
              <input className="input" placeholder="Search e.g. wheat, gehu, urea" value={catQ} onChange={e=>searchCat(e.target.value)} />
              {edit.catalog_name && <p className="text-xs text-brand mt-1">{t('Linked:')} {edit.catalog_name}</p>}
              {catResults.length>0 && (
            ...
            <div className="grid grid-cols-2 gap-3">
              <Inp label={t('SKU')} v={edit.sku} on={v=>setEdit({...edit,sku:v})} />
              <Inp label={t('Unit')} v={edit.unit} on={v=>setEdit({...edit,unit:v})} />
            </div>
            <Inp label={t('Name')} v={edit.name} on={v=>setEdit({...edit,name:v})} />
            <div className="grid grid-cols-2 gap-3">
      ...
      {pricing && (
        <Modal title={t('Pricing')+' — '+pricing.product.name} onClose={()=>setPricing(null)} onSave={savePricing}>
          <p className="text-xs text-slate-500 mb-3">Leave blank to use base price (₹{Number(pricing.product.base_price).toFixed(2)}).</p>
          {pricing.list.length===0 && <p className="text-sm text-slate-400">{t('No partner distributors yet.')}</p>}
          <div className="space-y-3">
