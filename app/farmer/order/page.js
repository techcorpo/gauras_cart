...
export default function FarmerOrder() {
  ...
  async function place(){
    ...
    setBusy(true);
    try { const r = await Orders.placeFarmerOrder({ distributor_id:did, season:season||null, items }); toast(t('Order')+' #'+r.order_number+' '+t('placed')); setTimeout(()=>router.replace('/farmer/dashboard'),1200); }
  ...
  return (
    <FodderShell role="farmer" title="Place a New Order" description="Choose a distributor serving your block, then select products.">
      ...
      <div className="card p-5 mb-5 grid md:grid-cols-2 gap-4">
        <div><div className="label">{t('Distributor')}</div>
        ...
        <div><div className="label">{t('Season')}</div>
          <select className="input" value={season} onChange={e=>setSeason(e.target.value)}>
            <option value="">{t('— Optional —')}</option>{['kharif','rabi','zaid','annual'].map(s=><option key={s} value={s} className="capitalize">{s}</option>)}
          </select>
        </div>
      </div>
      ...
      <div className="card overflow-x-auto mb-5">
        <table className="w-full text-sm">
          <thead className="bg-[#fbfdfc] dark:bg-[#101a15] text-slate-500 text-xs uppercase">
            <tr>{['Product','Manufacturer','Unit','Price','Quantity','Subtotal'].map(h=><th key={h} className="text-left p-3">{t(h)}</th>)}</tr>
          </thead>
          <tbody>
            {products.length===0 && <tr><td colSpan={6} className="p-8 text-center text-slate-400">{t('Select a distributor to see products.')}</td></tr>}
            {products.map(p=>(
        ...
        <div className="flex justify-between items-center p-4 border-t border-[#eef2ef] dark:border-[#24332b] bg-[#fbfdfc] dark:bg-[#101a15]">
          <span>{t('Order Total')}</span><span className="text-xl font-extrabold text-brand">₹{total.toFixed(2)}</span>
        </div>
      </div>

      <div className="text-right"><button className="btn btn-primary" disabled={!canPlace||busy} onClick={place}>{busy?t('Placing…'):t('Place Order')}</button></div>
    </FodderShell>
  );
}
