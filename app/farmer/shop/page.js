      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <aside className="lg:col-span-1 card p-5 h-fit space-y-6">
          <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-[#24332b]">
            <h3 className="font-black text-sm flex items-center gap-2"><Filter className="w-4 h-4 text-brand" /> FILTERS</h3>
            <button onClick={()=>{setCategory('all');setDistributor('all');setQ('');}} className="text-xs text-brand font-bold hover:underline">Clear</button>
          </div>

          <div>
            <h4 className="font-bold text-xs text-slate-500 uppercase tracking-wider mb-2">Category</h4>
            <div className="space-y-1.5">
              <FilterBtn active={category==='all'} onClick={()=>setCategory('all')} label="All Categories" />
              {categories.map(c => <FilterBtn key={c} active={category===c} onClick={()=>setCategory(c)} label={c} />)}
            </div>
          </div>

          <div>
            <h4 className="font-bold text-xs text-slate-500 uppercase tracking-wider mb-2">Distributor</h4>
            <div className="space-y-1.5">
              <FilterBtn active={distributor==='all'} onClick={()=>setDistributor('all')} label="All Distributors" />
              {distributors.map(([id,name]) => <FilterBtn key={id} active={distributor===String(id)} onClick={()=>setDistributor(String(id))} label={name} />)}
            </div>
          </div>

          <div>
            <h4 className="font-bold text-xs text-slate-500 uppercase tracking-wider mb-2">Sort</h4>
          ...
          <div className="bg-gradient-to-br from-brand-deep to-brand p-4 rounded-xl text-white text-xs space-y-1">
            <span className="bg-amber-400 text-amber-950 font-black px-2 py-0.5 rounded text-[9px] uppercase">Gauras Assured</span>
            <p className="font-bold text-sm">Quality Guarantee</p>
            <p className="text-white/80">Order genuine inputs from verified manufacturers via your local distributors.</p>
          </div>
        </aside>
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="bg-gradient-to-r from-brand to-brand-dark p-6 rounded-3xl text-white flex flex-col md:flex-row justify-between items-center gap-4">
            <div className="space-y-1.5 text-center md:text-left">
              <span className="bg-amber-400 text-slate-950 text-xs font-black px-3 py-1 rounded-full uppercase tracking-wider">Direct from Distributors</span>
              <h2 className="text-2xl font-black">Shop Agricultural Inputs</h2>
              <p className="text-sm text-white/85 max-w-lg">Seeds, fertilizers and more — delivered through your block's distributors.</p>
            </div>
            <div className="bg-white/10 backdrop-blur px-6 py-4 rounded-2xl text-center border border-white/10">
              <p className="text-xs uppercase tracking-wider text-white/80">Products</p>
          ...
          <div className="flex justify-between items-center pb-2 border-b border-slate-200 dark:border-[#24332b]">
            <p className="text-sm font-semibold text-slate-600 dark:text-slate-300">Showing <span className="font-bold">{list.length}</span> products</p>
          </div>
          ...
          {loading ? (
            ...
            <div className="card p-12 text-center space-y-3">
              ...
              <p className="text-sm text-slate-500">Try clearing filters or a different search.</p>
            </div>
          ) : (
                    ...
                    <div className="p-4 flex-1 flex flex-col justify-between space-y-3">
                      <div className="space-y-1.5">
                        <div className="flex items-center justify-between text-[11px] text-slate-500">
                          <span>Unit: <strong className="text-slate-700 dark:text-slate-300">{p.unit}</strong></span>
                      ...
                      <div className="space-y-2 pt-3 border-t border-slate-100 dark:border-[#24332b]">
                        <div className="flex items-baseline justify-between">
                          ...
                          <span className="text-[10px] text-brand font-bold bg-brand-light dark:bg-[#1c2a22] px-2 py-0.5 rounded">Free Delivery</span>
                        </div>
                        <p className="text-[10px] text-slate-400">By <span className="font-bold text-slate-600 dark:text-slate-300">{p.manufacturer_name}</span></p>
                        <button onClick={()=>{cart.add(p); toast('Added to cart');}}
