...
export default function RegisterRole() {
  ...
  // Field auto-translates its label via t().
  const Field = ({ label, children }) => (
    <div><div className="label">{t(label)}</div>{children}</div>
  );
    ...
    <div className="min-h-screen bg-slate-900 py-8 px-4">
      ...
      <div className="max-w-3xl mx-auto">
        <Link href="/register" className="inline-flex items-center gap-1.5 text-sm text-slate-300 hover:text-white mb-4">
          <ArrowLeft className="w-4 h-4" /> {t('Back to roles')}
        </Link>
        ...
        <div className="p-7 bg-gradient-to-r from-brand-deep to-brand-dark text-white flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-black">Gauras Mart — {t(TITLES[role])}</h1>
            <p className="text-sm text-white/75">{t('Fill the details below to register.')}</p>
          </div>
        </div>

        <form onSubmit={submit} className="p-7 space-y-4">
          {role === 'farmer' && (<>
            <Field label="Name"><input className="input" onChange={e=>set('farmerName',e.target.value)} placeholder={t('Name')} /></Field>
            <Field label="Address"><input className="input" onChange={e=>set('address',e.target.value)} placeholder={t('Address')} /></Field>
            <div className="grid md:grid-cols-2 gap-4">
              <Field label="Contact Number"><input className="input" onChange={e=>set('phone',e.target.value)} placeholder={t('Contact Number')} /></Field>
          ...
          {role === 'manufacturer' && (<>
            <Field label="Company Name"><input className="input" onChange={e=>set('companyName',e.target.value)} placeholder={t('Company Name')} /></Field>
            <Field label="Address"><input className="input" onChange={e=>set('address',e.target.value)} placeholder={t('Address')} /></Field>
            <div className="grid md:grid-cols-2 gap-4">
              <Field label="Contact Person"><input className="input" onChange={e=>set('contactPerson',e.target.value)} placeholder={t('Name')} /></Field>
              <Field label="Contact Number"><input className="input" onChange={e=>set('phone',e.target.value)} placeholder={t('Contact Number')} /></Field>
            </div>
            <Field label="District">
              <select className="input" onChange={e=>onDistrict(e.target.value)}>
          ...
          {role === 'distributor' && (<>
            <Field label="Distributorship Name"><input className="input" onChange={e=>set('distributorshipName',e.target.value)} placeholder={t('Distributorship Name')} /></Field>
            <Field label="Address"><input className="input" onChange={e=>set('address',e.target.value)} placeholder={t('Address')} /></Field>
            <div className="grid md:grid-cols-2 gap-4">
              <Field label="Registration Number"><input className="input" onChange={e=>set('registrationNumber',e.target.value)} placeholder={t('Registration Number')} /></Field>
              <Field label="Chairman Name"><input className="input" onChange={e=>set('chairmanName',e.target.value)} placeholder={t('Chairman Name')} /></Field>
            </div>
            <div className="grid md:grid-cols-2 gap-4">
              <Field label="Contact Person"><input className="input" onChange={e=>set('contactPerson',e.target.value)} placeholder={t('Contact Person')} /></Field>
              <Field label="Contact Number"><input className="input" onChange={e=>set('phone',e.target.value)} placeholder={t('Contact Number')} /></Field>
            </div>
            <Field label="Email (optional)"><input className="input" onChange={e=>set('email',e.target.value)} placeholder="you@example.com" /></Field>
            ...
            <div>
              <div className="label">{t('Service Territory (select blocks)')}</div>
              <div className="border border-[#e0e8e3] dark:border-[#2c4034] rounded-lg p-3 space-y-3 max-h-64 overflow-y-auto">
                ...
                {allBlocksByDistrict.length === 0 && <p className="text-sm text-slate-400">{t('Loading blocks…')}</p>}
              </div>
            </div>
          </>)}
          ...
          {role === 'farmer' ? (
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <Field label="Password"><input className="input" type="password" onChange={e=>set('password',e.target.value)} placeholder={t('Password')} /></Field>
              <Field label="Confirm Password"><input className="input" type="password" onChange={e=>set('confirm',e.target.value)} placeholder={t('Confirm Password')} /></Field>
            </div>
          )}
          ...
          <div className="flex justify-between items-center pt-2">
            <Link href="/register" className="text-sm text-slate-500">← {t('Back to roles')}</Link>
            <button className="bg-amber-400 hover:bg-amber-500 disabled:bg-slate-300 text-slate-950 font-black px-6 py-2.5 rounded-xl transition" disabled={busy}>{busy ? t('Submitting…') : t('Register')}</button>
          </div>
        </form>
        </div>
      </div>
