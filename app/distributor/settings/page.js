  ...
  return (
    <FodderShell role="distributor" title="Settings" description="Service territory and manufacturer partners.">
      ...
      <div className="card p-6 mb-6">
        <h2 className="font-bold mb-4">{t('Service Territory')}</h2>
        ...
        <div className="mt-4 text-right"><button className="btn btn-primary" onClick={async()=>{try{await Distributor.saveBlocks(Array.from(selB));toast(t('Territory saved'));}catch(e){toast(e.message);}}}>{t('Save Territory')}</button></div>
      </div>

      <div className="card p-6">
        <div className="flex items-center justify-between mb-1 flex-wrap gap-2">
          <h2 className="font-bold">{t('Manufacturer Partners')}</h2>
        ...
        {exclusive ? (
          ...
          <p className="text-sm text-slate-500 mb-4">{t('Select the manufacturers you buy from.')}</p>
        )}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          ...
          {mfrs.length===0 && <p className="text-sm text-slate-400">{t('No manufacturers available.')}</p>}
        </div>
        <div className="mt-4 text-right"><button className="btn btn-primary" onClick={async()=>{try{await Distributor.saveManufacturers(Array.from(selM));toast(t('Partners saved'));}catch(e){toast(e.message);}}}>{t('Save Partners')}</button></div>
      </div>
    </FodderShell>
  );
}
