    ...
    <FodderShell role="distributor" title="Orders" description="Aggregate farmer demand into purchase orders.">
      <div className="flex gap-2 mb-4">
        {[['incoming','Farmer Orders'],['pos','My Purchase Orders']].map(([k,l])=>(
          <button key={k} onClick={()=>setTab(k)} className={`px-4 py-2 rounded-lg text-sm font-bold ${tab===k?'bg-brand text-white':'btn-secondary'}`}>{t(l)}</button>
        ))}
      </div>
        ...
        <div className="card overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-[#fbfdfc] dark:bg-[#101a15] text-slate-500 text-xs uppercase">
              <tr>{['','Farmer','Block','Product','Manufacturer','Qty','Status','Farmer Payment'].map((h,i)=><th key={i} className="text-left p-3">{h?t(h):''}</th>)}</tr>
            </thead>
            <tbody>
              {items.length===0 && <tr><td colSpan={8} className="p-8 text-center text-slate-400">{t('No farmer orders yet.')}</td></tr>}
              {items.map(it=>(
          ...
          <div className="flex items-center gap-3 p-4 border-t border-[#eef2ef] dark:border-[#24332b] bg-[#fbfdfc] dark:bg-[#101a15] flex-wrap">
            <span className="text-xs text-slate-500">{t('Select items from one manufacturer, then:')}</span>
            <button className="btn btn-primary" disabled={!canAgg} onClick={aggregate}>{t('Create Purchase Order →')}</button>
            {sel.size>0 && <span className={`text-xs font-bold ${mfrs.size>1?'text-rose-600':'text-brand'}`}>{mfrs.size>1?t('⚠ One manufacturer only'):sel.size+' '+t('selected')}</span>}
          </div>
        </div>
      )}
      ...
      {tab==='pos' && (
        <div className="card overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-[#fbfdfc] dark:bg-[#101a15] text-slate-500 text-xs uppercase">
              <tr>{['PO #','Manufacturer','Season','Total','Status','Payment','Date'].map(h=><th key={h} className="text-left p-3">{t(h)}</th>)}</tr>
            </thead>
            <tbody>
              {pos.length===0 && <tr><td colSpan={7} className="p-8 text-center text-slate-400">{t('No purchase orders yet.')}</td></tr>}
              {pos.map(p=>(
                <tr key={p.id} className="border-t border-[#f0f3f1] dark:border-[#24332b]">
                  ...
                  <td className="p-3"><span className={`pill ${pill(p.status)}`}>{t(p.status)}</span></td>
                  <td className="p-3"><span className={`pill ${p.payment_status==='paid'?'bg-[#e5f5eb] text-brand':'bg-[#fff4dc] text-[#9a6a12]'}`}>{t(p.payment_status)}</span></td>
